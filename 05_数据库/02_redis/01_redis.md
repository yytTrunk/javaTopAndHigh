...
#### 5.2 redis高可用集群模式（Redis Cluster）
...
分布式数据库 首先要解决把 整个数据集 按照 分区规则*映射到 多个节点 的问题，即把 数据集划分到 多个节点 上，每个节点负责 整体数据 的一个 子集。
...
### 七 redis基本特性，事务、生成时间、排序、消息队列、管道
