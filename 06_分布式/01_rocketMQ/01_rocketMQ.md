...
kafka：
	...
	5、应用场景：   适当丢失数据没有关系、吞吐量要求高、不需要太多的高级功能的场景，适用于大数据方向日志采集场景。
RocketMQ：
	1、开发语言：     java开发
...
- 同步发送
  ...
  是指消息发送方发出一条消息后，会在收到服务端返回响应之后才发下一条消息的通讯方式。
...
![img](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/1579264061/p177406.png)
...
3. 发送方开始执行本地事务逻辑。
4. 发送方根据本地事务执行结果向服务端提交二次确认（Commit或是Rollback），本地执行成功，服务端收到Commit状态则将半事务消息标记为可投递，订阅方最终将收到该消息；本地执行失败，服务端收到Rollback状态则删除半事务消息，订阅方将不会接受该消息。
5. 当MQ服务端未收到消息时，会去回查本地事务状态
...
<img src=".\img\01_04.png" alt="消息存储整体架构" style="zoom: 75%;" />
...
![img](https://static001.geekbang.org/infoq/c0/c03c4e3ffab1de959cd255b9ac20328f.jpeg?x-oss-process=image/resize,p_80/auto-orient,1)
...
https://xie.infoq.cn/article/ef5e177962cf797b8bd6ded82



### 8.13 RocketMQ消费模式有几种？
...
2、退款失败问题，订单支付系统完成公司系统内部业务的核心流程，发送消息到MQ，使用一个专门的系统去处理和第三方支付系统的交互。
...
https://bbs.huaweicloud.com/blogs/186029
...
- 消费者消费消息时，消费成功后，还没来得及返回ACK信息，挂掉了，导致其它消费者再消费

- 消费者消费失败，进行重试，生成者又将消息重新发送，允许这种方式

#### 解决方式

- 将处理消息接口设计为幂等，即使多次调用，返回结果也一致
- 使用防重表（redis/mysql），每个消息具备唯一标识，处理过的消息，就不会再处理了



### 9.7 RocketMQ Broker中的消息被消费后会立即删除吗？
...
[消息队列MQ DEMO](https://www.bilibili.com/video/BV1Qt411g76Z?p=10&spm_id_from=pageDriver)
